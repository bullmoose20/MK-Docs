...
??? blank "`imdb_awards` - Collections based on IMDb Events by Year.<a class="headerlink" href="#imdb-awards" title="Permanent link">¶</a>"
    ...
    **`data` Value:** [Dictionary](../komet/yaml.md#dictionaries) of Attributes
    ...
    **Default `title_format`:** `Best <<library_type>> of the <<key_name>>`
    ...
    ??? tip "Default Templates (click to expand)"
        ...
        === "Show Default"
            ...
            Shows don't inherently have a decade attribute so Kometjust passes all years from the decade as a list.
...
??? blank "`custom` - Collections based on given values.<a class="headerlink" href="#custom" title="Permanent link">¶</a>"
    ...
    **`data` Value:** [Dictionary](../komet/yaml.md#dictionaries) with the keys being the `dynamic key` and the values 
    being the `key name`
    ...
    ???+ example "Example"
        ...
        templates:
          streaming:
            ...
            url_poster: https://raw.githubusercontent.com/meisnate12/komet-Images/master/streaming/<<key_name_encoded>>.jpg
